...
open class JXSegmentedCustomDataSource: JXSegmentedTitleDataSource {
    ...
    /// 需要和titles数组数量一致，没有图片角标填空！！！
    open var cornerMarkerImages = [String]()
    ...
    open override func preferredRefreshItemModel(_ itemModel: JXSegmentedBaseItemModel, at index: Int, selectedIndex: Int) {
        ...
        guard let itemModel = itemModel as? JXSegmentedCustomItemModel else {
            return
        }
        ...
        itemModel.imageInfo = cornerMarkerImages[index]
        itemModel.imageSize = imageSize
        itemModel.imageOffsetX = imageOffsetX
        itemModel.imageOffsetY = imageOffsetY
        
        if let stringCount = itemModel.imageInfo?.count, stringCount > 0 {
            itemModel.type = .image
        }
        else {
            itemModel.type = .number
        }
    }
    
    //MARK: - JXSegmentedViewDataSource
    open override func registerCellClass(in segmentedView: JXSegmentedView) {
        segmentedView.collectionView.register(JXSegmentedCustomCell.self, forCellWithReuseIdentifier: "cell")
    }
